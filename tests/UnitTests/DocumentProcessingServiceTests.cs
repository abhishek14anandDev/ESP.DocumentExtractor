...
public sealed class DocumentProcessingServiceTests
{
    ...
    public async Task ProcessAsync_ShouldPersistAndMoveBlob_WhenExtractionSucceeds()
    {
        ...
        var logger = new Mock<ILogger<DocumentProcessingService>>();
        ...
        auditRepository.Setup(x => x.SaveAsync(It.IsAny<Domain.Entities.DocumentProcessingAudit>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        historyRepository.Setup(x => x.SaveAsync(It.IsAny<Domain.Entities.BlobProcessingHistory>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        auditRepository.Verify(x => x.SaveAsync(It.IsAny<Domain.Entities.DocumentProcessingAudit>(), It.IsAny<CancellationToken>()), Times.Once);
        historyRepository.Verify(x => x.SaveAsync(It.IsAny<Domain.Entities.BlobProcessingHistory>(), It.IsAny<CancellationToken>()), Times.Once);
    }
}
